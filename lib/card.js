class ProductCard extends HTMLElement {
  ...
  render() {
    ...
    let iconHTML = '';
    if (iconType === 'emote') {
        iconHTML = `<icon-popover icon="😭"><div>Emote packs come with 8 emoji!</div></icon-popover>`;
    } else if (iconType === 'pngtuber') {
        iconHTML = `<icon-popover icon="🎙️"><div>PNGTubers come with 4 PNGs by default.</div></icon-popover>`;
    } else if (iconType === 'sticker') {
        iconHTML = `<icon-popover icon="📎"><div>Digital sticker packs have multiple stickers.</div></icon-popover>`;
    }
